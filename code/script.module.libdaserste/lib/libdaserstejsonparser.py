...
def getSearch(s):
	#url = 'http://www.daserste.de/dasersteapp/app/index~program_pd-'+day+'.json'
	url = 'http://www.daserste.de/dasersteapp/index~search_documentTypes-video_s-broadcastDate_dir-desc_f-dateRange_fk-1YEAR%7C14DAYS_searchText-'+urllib.quote_plus(s)+'.json'
	response = libMediathek.getUrl(url)
...
def getVideo(url):
	...
	try:
		...
		d['metadata'] = {'name':j['headline'], 'plot': j['copytext'][0]['text'], 'thumb':thumb, 'duration':str(j['videoDuration'])}
	except: pass
	if 'subtitles' in j:
		d['subtitle'] = []
