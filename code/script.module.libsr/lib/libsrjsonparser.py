...
def _getShowsSubset(url):
	...
	for item in j:
		...
		#d['s_id'] = item['s_id']
		d['plot'] = item['s_beschreibung']
		d['urlargs'] = json.dumps({'az': '1','sendereihe': item['s_id']})
		d['thumb'] = 'http://sr-mediathek.de/img/sendungen/'+item['s_id']+'~_v-sr_169_600.jpg'
...
def getTopics():
	...
	for item in j:
		...
		d['plot'] = item['t_name']
		#d['thumb'] = 'http://sr-mediathek.de/img/sendungen/'+item['t_kurz']+'~_v-sr_169_600.jpg'
		d['mode'] = 'libSrListVideos'
		d['_type'] = 'dir'
		if item['t_anzahl'] != '0':
			l.append(d)
