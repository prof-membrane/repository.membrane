...
def getVideo(id):
	...
	for item in j['playlist']:
		if item != 'config' and 'type' in j['playlist'][item]:
			if j['playlist'][item]['type'] == 'application/x-mpegURL':
				d['media'].append({'url':j['playlist'][item]['src'], 'type': 'video', 'stream':'HLS'})
			elif j['playlist'][item]['type'] == 'audio/mp3':
				d['media'].append({'url':j['playlist'][item]['src'], 'type': 'audio', 'stream':'audio'})
